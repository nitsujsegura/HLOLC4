...
if (isset($_SESSION['admin_id'])) {
    ...
    if (empty($first_name)) {
        ...
        $sql = "UPDATE students SET
                username = ?, first_name=?, last_name=?, grade=?, address=?, lrn=?, gender = ?, section=?, religion=?, date_of_birth=?, guardian_parent_first_name=?,guardian_parent_last_name=?,guardian_parent_phone_number=?
                WHERE student_id=?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$uname,$first_name, $last_name, $grade, $address,$lrn,$gender,$section, $religion, $date_of_birth, $parent_fname, $parent_lname,$parent_phone_number, $student_id]);
